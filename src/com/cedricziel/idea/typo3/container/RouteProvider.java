...
public class RouteProvider {
    ...
    private void collectRoutes(Project project) {
        ...
        containingFiles.removeIf(virtualFile -> {
            return !(virtualFile.getName().contains("Routes.php") || virtualFile.getName().contains("AjaxRoutes.php"));
        });

        for (VirtualFile projectFile : containingFiles) {
            PsiFile psiFile = PsiManager.getInstance(project).findFile(projectFile);
            if (psiFile != null) {
                psiFile.accept(new RouteDefinitionParserVisitor(routes, ajaxRoutes));
            }
        }
    }
